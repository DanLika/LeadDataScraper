...
| `/monitoring` (POST + OPTIONS)| `app/monitoring/route.ts`                     | Sentry envelope tunnel (manual fallback to webpack-plugin virtual route). Edge runtime. `Content-Type: application/x-sentry-envelope` only (415 else), 1 MB cap (413), envelope `dsn` must match `NEXT_PUBLIC_SENTRY_DSN` (403 anti-abuse), 5 s `AbortSignal.timeout` (502), 204 no-op when DSN unset. Public-allowlisted in middleware. |
