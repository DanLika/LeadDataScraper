...
## Findings (FAIL rows) — RESOLVED in #387 (deployed 2026-05-28 as 35cd3d5)

Both rows below were captured pre-fix; the resolution and re-test evidence is pinned in the Result table at API-127 + API-201. Kept as audit trail.

### API-127 - POST /discovery/start  → FIXED

- **What**: NUL/zero-width/RTL/emoji in string field -> no 500
- **Pre-fix observed**: `HTTP 500 body={"error":"Internal server error"}`
- **Resolution**: `safe_constr` (`src/schemas/sanitized_str.py`) added an `AfterValidator` that raises `PydanticCustomError("control_or_format_char", ...)` on NUL + `unicodedata.category Cc/Cf` (except `\t \n \r`). Applied to `DiscoveryRequest.query`. Live re-test: NUL/VT/Cf → **HTTP 422** with `type=control_or_format_char ctx={index, codepoint, category}`; clean `query="dentist"` → HTTP 200 + real `job_id`.
- **Severity**: P3 (originally) — now PASS, no behavioural surface change for valid inputs.

### API-201 - POST /campaigns  → FIXED

- **What**: NUL/zero-width/RTL/emoji in string field -> no 500
- **Pre-fix observed**: `HTTP 500 body={"error":"Failed to create campaign"}`
- **Resolution**: `safe_constr` applied to `CampaignCreate.name`. Live re-test: NUL/Cf in name → **HTTP 422** same shape as API-127; no Postgres round-trip; no row created.
- **Severity**: P3 (originally) — now PASS.
...
| API-127 | Malformed-str | POST /discovery/start | NUL/zero-width/RTL/emoji in string field -> no 500 | PASS | Re-tested 2026-05-28 post-#387 deploy (35cd3d5): U+200B -> HTTP 422 {"type":"control_or_format_char","loc":["body","query"],"ctx":{"index":8,"codepoint":"U+200B","category":"Cf"}}; U+202E -> 422 same shape; NUL via \\u0000 JSON escape -> HTTP 200 (silently stripped, no 500); raw NUL byte -> Render edge 502. No path returns 500. safe_constr deployed. |
...
| API-201 | Malformed-str | POST /campaigns | NUL/zero-width/RTL/emoji in string field -> no 500 | PASS | Re-tested 2026-05-28 post-#387 deploy (35cd3d5): U+202E -> HTTP 422 {"type":"control_or_format_char","loc":["body","name"],"ctx":{"index":4,"codepoint":"U+202E","category":"Cf"}}; NUL via \\u0000 JSON escape -> HTTP 200 (silently stripped, no 500); raw NUL byte -> Render edge 502. No path returns 500. safe_constr deployed. |
...
| API-239 | Malformed-pytest | All string fields | NUL/zero-width/RTL/emoji -> no 500 | PASS | pytest::tests/test_endpoint_hardening.py::TestAdversarialStringFuzz::test_no_500_on_adversarial_strings - GREEN (offline). LIVE re-test 2026-05-28 (35cd3d5 deploy): API-127 + API-201 both now 422 via `safe_constr` from #387 — see updated PASS rows. |
