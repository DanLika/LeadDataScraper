...
class TestTemplateRendererContentType:
    ...
    def test_html_mode_escapes_ampersand(self):
        assert render("{{ company }}", {"company": "X&Y"}, content_type="html") == "X&amp;Y"
...
_WINDOW_CASES = [
    ...
    (datetime(2026, 6, 2, 7, 0, tzinfo=timezone.utc), True, "Tuesday 09:00 local (start inclusive)"),
    (datetime(2026, 6, 2, 15, 0, tzinfo=timezone.utc), False, "Tuesday 17:00 local (end exclusive)"),
...
def test_send_window_returns_next_open_when_outside():
    ...
    assert result.next_window_start_utc == datetime(2026, 6, 1, 7, 0, tzinfo=timezone.utc)
...
class TestVariantSelector:
    ...
    def test_deterministic_seed_honored_under_env_gate(self, monkeypatch):
        ...
        a = _V(id="a", variant_label="A", weight=50)
        b = _V(id="b", variant_label="B", weight=50)
        picks = {select_variant([a, b], deterministic_seed="lead-42").id for _ in range(10)}
    ...
    def test_deterministic_seed_ignored_without_env_gate(self, monkeypatch):
        ...
        ids = {select_variant([a, b], deterministic_seed="lead-42").id for _ in range(1000)}
        assert ids == {"a", "b"}
...
class TestSequenceAdvancer:
    ...
    def test_sent_event_advances_always_branch(self):
        ...
        res = self._run(advance_to_next_step(
            current_message=_base_message(),
            step_repo=repo, message_repo=msgs, event_type="sent",
        ))
    ...
    def test_replied_event_skips_always_branch(self):
        ...
        res = self._run(advance_to_next_step(
            current_message=_base_message(),
            step_repo=repo, message_repo=msgs, event_type="replied",
        ))
        assert res.advanced is False
    ...
    def test_out_of_window_send_bumps_scheduled_at(self):
        ...
        res = self._run(advance_to_next_step(
            current_message=_base_message(),
            step_repo=repo, message_repo=msgs, event_type="sent", sent_at=sat,
        ))
    ...
    def test_thread_with_prior_wires_in_reply_to(self):
        ...
        repo, msgs = _FakeStepRepo([s1, s2]), _FakeMsgRepo()
        self._run(advance_to_next_step(
            current_message=_base_message(),
            step_repo=repo, message_repo=msgs, event_type="sent",
        ))
...
def _insert_seq_step(cur, seq_name="smoke-seq", step_kw=None):
    ...
    cur.execute(
        ...
        (str(step_id), str(seq_id), defaults["channel"], defaults["delay_days"],
         defaults["delay_hours"], defaults["branch_condition"],
...
class TestSequenceStepsChecks:
    ...
    def test_window_inverted_rejected(self, rollback_tx):
        ...
        with rollback_tx.cursor() as cur:
            with pytest.raises(psycopg.errors.CheckViolation):
                _insert_seq_step(
                    cur, step_kw={"send_window_start": "17:00", "send_window_end": "09:00"},
                )
...
def _insert_variant(cur, step_id, **kw):
    ...
    cur.execute(
        ...
        (str(uuid.uuid4()), str(step_id), defaults["variant_label"],
         defaults["subject_template"], defaults["body_template"],
         defaults["content_type"], defaults["weight"]),
    )
...
class TestSequenceVariantsChecks:
    ...
    def test_text_and_html_variants_both_insert(self, rollback_tx):
        with rollback_tx.cursor() as cur:
            ...
            _insert_variant(cur, step_id, variant_label="B", content_type="html",
