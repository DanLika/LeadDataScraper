...
class TestFix1_SSLBypassRemoved(unittest.TestCase):
    ...
    def test_no_cert_none_in_seo_audit(self):
        ...
        seo_audit_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "src", "scrapers", "seo_audit.py"
        )
...
class TestFix2_UUIDTempFilenames(unittest.TestCase):
    ...
    def test_uuid_import_present(self):
        ...
        self.assertIn(
            "import uuid", content, "UUID import is missing from backend/main.py!"
        )
...
class TestFix3_APIKeyLeakPrevention(unittest.TestCase):
    ...
    def test_no_api_key_in_error_messages(self):
        ...
        router_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "src", "core", "agentic_router.py"
        )
        ...
        error_returns = re.findall(
            r'return\s*\{["\']error["\']\s*:\s*["\']([^"\']+)', content
        )

        for msg in error_returns:
            self.assertNotIn(
                "GEMINI_API_KEY", msg, f"API key name leaked in error message: '{msg}'"
            )
            self.assertNotIn(
                "SUPABASE", msg, f"Supabase key leaked in error message: '{msg}'"
            )
            self.assertNotIn(
                "API_SECRET", msg, f"API secret leaked in error message: '{msg}'"
            )
...
class TestFix4_CORSWildcardGuard(unittest.TestCase):
    ...
    def test_wildcard_guard_code_exists(self):
        ...
        self.assertIn('origin != "*"', content, "Wildcard CORS strip not found!")
...
class TestFix5_SQLInjectionRegex(unittest.TestCase):
    ...
    def test_regex_uses_backslash_Z(self):
        ...
        helper_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "src", "utils", "supabase_helper.py"
        )
    ...
    def test_valid_column_names_pass(self):
        ...
        for name in valid_names:
            self.assertIsNotNone(
                pattern.match(name), f"Valid column name '{name}' was rejected!"
            )
    ...
    def test_dollar_sign_vulnerability_demonstration(self):
        ...
        self.assertIsNone(
            new_result, "The new \\Z pattern should NOT match (demonstrating fix)"
        )
...
class TestFix6_ParallelEnrichmentFetching(unittest.TestCase):
    ...
    def test_asyncio_gather_in_enrichment(self):
        ...
        self.assertIn("fetch_page", content, "fetch_page helper function not found!")
    ...
    def test_concurrent_fetch_preserves_results(self):
        ...
        self.assertIn(
            "for res in results:", content, "Results not being iterated after gather!"
        )
        self.assertIn(
...
class TestFix7_SmartWaitInDiscovery(unittest.TestCase):
    ...
    def test_no_hardcoded_sleep_for_initial_load(self):
        ...
        self.assertNotIn(
            "5", sleep_calls, "asyncio.sleep(5) still exists — blind wait not replaced!"
        )
    ...
    def test_timeout_fallback_exists(self):
        ...
        self.assertIn(
            "PlaywrightTimeoutError", content, "Timeout error handling not found!"
        )
        self.assertIn(
            "timeout=10000",
            content,
...
class TestFix9_PrecompiledRegex(unittest.TestCase):
    ...
    def test_compiled_patterns_at_module_level(self):
        ...
        hunter_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "src", "processors", "leadhunter.py"
        )
...
class TestFix10_ReputationSegmentHelper(unittest.TestCase):
    ...
    def test_helper_method_exists(self):
        ...
        hunter = LeadHunter()
        self.assertTrue(
            hasattr(hunter, "_get_reputation_segment"),
    ...
    def test_segment_lead_integration(self):
        ...
        lead2 = {
            "pain_points": "website looks outdated",
            "rating": "3.5",
            "reviews": "50",
        }
        ...
        lead3 = {
            "pain_points": "limited online presence",
            "rating": "4.8",
            "reviews": "3",
        }
...
class TestEndToEnd_SEOAudit(unittest.TestCase):
    ...
    def test_audit_invalid_url_returns_error(self):
        ...
        result = asyncio.run(perform_seo_audit_async(""))
...
class TestEndToEnd_SegmentLead(unittest.TestCase):
    ...
    def test_all_segments_reachable(self):
        ...
        test_cases = [
            (
                {"pain_points": "critical vulnerability", "outreach_score": 0},
                "Security/Critical Fix",
            ),
            (
                {"pain_points": "slow load time 8s", "outreach_score": 0},
                "Performance Optimization",
            ),
            (
                {"pain_points": "missing viewport meta", "outreach_score": 0},
                "Mobile Experience",
            ),
            (
                {"pain_points": "no pixel tracking", "outreach_score": 0},
                "Marketing Analytics",
            ),
            (
                {
            ...
            (
                {
                    "pain_points": "",
                    "target_clients": "enterprise companies",
                    "outreach_score": 0,
                },
                "Enterprise B2B",
            ),
            (
                {
                    "pain_points": "",
                    "target_clients": "local homeowners",
                    "outreach_score": 0,
                },
                "Local SMB",
            ),
