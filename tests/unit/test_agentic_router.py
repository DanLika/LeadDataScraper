...
class TestAgenticRouterExecuteTask(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        with (
            patch("src.core.agentic_router.os.getenv", return_value="fake_key"),
    ...
    async def test_execute_database_query(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "DATABASE_QUERY", "params": {"query_text": "hello"}}
        )
    ...
    async def test_execute_status_check(self, mock_method):
        ...
        result = await self.router.execute_task({"task": "STATUS_CHECK"})
    ...
    async def test_execute_seo_audit_single(self, mock_auditor_class):
        ...
        result = await self.router.execute_task(
            {"task": "SEO_AUDIT", "params": {"unique_key": "test_key"}}
        )

        mock_auditor.audit_single_lead.assert_called_once_with(
            {"unique_key": "test_key"}
        )
        self.assertEqual(
            result,
    ...
    async def test_execute_seo_audit_massive(self, mock_orchestrator_class):
        ...
        result = await self.router.execute_task({"task": "SEO_AUDIT"})

        mock_orchestrator.run_massive_pipeline.assert_called_once_with(tasks=["audit"])
        self.assertEqual(
            result,
            {"message": "Massive SEO Audit pipeline started.", "job_id": "job_123"},
        )
    ...
    async def test_execute_outreach_draft(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "OUTREACH_DRAFT", "params": {"unique_key": "test_key"}}
        )
    ...
    async def test_execute_get_insights(self, mock_method):
        ...
        result = await self.router.execute_task({"task": "GET_INSIGHTS"})
        mock_method.assert_called_once_with()
    ...
    async def test_execute_data_merge(self, mock_method):
        ...
        result = await self.router.execute_task({"task": "DATA_MERGE"})
        mock_method.assert_called_once_with()
    ...
    async def test_execute_deep_hunt(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "DEEP_HUNT", "params": {"unique_key": "key"}}
        )
    ...
    async def test_execute_run_massive_pipeline(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "RUN_MASSIVE_PIPELINE", "params": {"filters": "high-risk"}}
        )
    ...
    async def test_execute_linkedin_draft(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "LINKEDIN_DRAFT", "params": {"unique_key": "key"}}
        )
        mock_method.assert_called_once_with({"unique_key": "key"})
    ...
    async def test_execute_discovery_search(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "DISCOVERY_SEARCH", "params": {"query": "pizza"}}
        )
    ...
    async def test_execute_deep_enrichment(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "DEEP_ENRICHMENT", "params": {"unique_key": "key"}}
        )
        mock_method.assert_called_once_with({"unique_key": "key"})
    ...
    async def test_execute_campaign_strategy(self, mock_method):
        ...
        result = await self.router.execute_task(
            {"task": "CAMPAIGN_STRATEGY", "params": {"filters": "high-risk"}}
        )
        mock_method.assert_called_once_with({"filters": "high-risk"})
        self.assertEqual(result, {"message": "planned"})

    async def test_execute_unknown_task(self):
        result = await self.router.execute_task({"task": "UNKNOWN_TASK"})
...
class TestAgenticRouter(unittest.IsolatedAsyncioTestCase):
    async def test_route_instruction_error_path(self):
        ...
        self.assertEqual(
            result.get("reasoning"), "Tool calling failed: Simulated AI Failure"
        )
