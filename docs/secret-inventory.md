...
| `SENTRY_DSN` (backend) | DSN string for the `lds-backend` Sentry project (project_id `4511473178574928`, org `o4511473167695873`, EU region). Same value on backend web service `srv-d89bisbbc2fs73f1pjpg` AND sweeper cron `crn-d8bi9137uimc73doo5v0`. | Read+write to one Sentry project's event ingest. Pointed at a SDK init only — no project admin access. | **Annual** unless leaked. On leak: revoke DSN in Sentry → Settings → Client Keys, issue a new one, re-PATCH on both Render services. Build-time env-PUT does NOT auto-redeploy — follow with explicit `POST /v1/services/<id>/deploys`. | Suspicious volume of events from unexpected User-Agent in Sentry dashboard. |
| `SENTRY_ENVIRONMENT` | Sentry tag for event filtering (`production` / `preview` / `development`). | Non-secret. | Update on env change. | Wrong tag → events show under wrong dashboard. |
| `SENTRY_TEST_ENABLED` | Gates `POST /_sentry/test` (404 when unset). | Operator-time-bounded verification only. | Always unset in prod. Set to `1` only during smoke, unset within minutes. | Long-lived `1` value → unauthenticated RuntimeError-spam vector. |
...
| `SENTRY_DSN` (frontend server) | DSN for the `lds-frontend` Sentry project (project_id `4511473196925008`). Used by `sentry.server.config.ts` + `sentry.edge.config.ts`. | Read+write to one Sentry project. | **Annual** unless leaked. Paired rotation with `NEXT_PUBLIC_SENTRY_DSN` (same value). | Same as backend DSN row. |
| `NEXT_PUBLIC_SENTRY_DSN` | DSN exposed to the browser SDK (`instrumentation-client.ts`). **Public by design** — `NEXT_PUBLIC_*` ships in the JS bundle. Same value as the server-side `SENTRY_DSN` above. | Browser-readable. Sentry rate-limits per-DSN so this is acceptable; compromise just means events keep flowing, not access escalation. | Paired with frontend `SENTRY_DSN`. | Sentry → Project → Stats: unexpected event volume. |
| `SENTRY_ORG` / `SENTRY_PROJECT` | Slugs for source-map upload at `next build`. | Build-time only, not present at runtime. | Update on Sentry org/project rename. | Source-map upload fails silently → minified stack traces in dashboard. |
| `SENTRY_AUTH_TOKEN` | User Auth Token from Sentry → Settings → Account → API → Auth Tokens. Scope: `project:releases` + `project:write`. Source-map upload during `next build`. **Currently UNSET on Render frontend** — stack traces stay minified until populated. | Project-level scope (NOT org admin). | **Annual** or on suspected leak. On leak: revoke the specific token in Sentry, mint replacement. | Source-map upload failure logs in Render build output. |
...
| `RENDER_API_KEY` | `post-deploy-smoke.yml`, `deploy-backend.yml`, `synthetic-monitor.yml`, ad-hoc local Mgmt API calls | Render API: deploy + read for ALL services on the account. **Long-lived bearer.** `rnd_`-prefix, 32 chars. | **Monthly** (next: 2026-06-22). On leak: rotate immediately, audit Render deploy history for unauthorized rollouts. If Render exposes per-service or scoped tokens (verify in dashboard), use the narrower form. **Local storage note**: lives in `~/.zshenv` only (NOT `~/.bookbed-secrets`); subprocess shells via `ctx_execute` do NOT inherit it — re-extract via `grep '^export RENDER_API_KEY=' ~/.zshenv \| sed 's/^export RENDER_API_KEY=//; s/^"//; s/"\$//'` at the top of each sandbox block. Consider mirroring into `~/.bookbed-secrets` to unify the secrets-source pattern. |
