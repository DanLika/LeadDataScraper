...
- **`dispatch_tick` per-lead error telemetry** (Issue #367, PR #371): `TickResult` carries 4 orthogonal counters — `failed` (provider-reported count from `push_leads.failed_count` PLUS pre-dispatch rejects), `swallowed` (per-lead `push_result.errors[]` rows we could NOT reconcile to a claimed `message_id`), `dispatched` (success), `errors[]` (tick-level tags). Distinct buckets, never merged. `_handle_per_lead_errors` ALWAYS logs raw `push_leads` error at INFO BEFORE reconciliation — operator sees every failure regardless of email-lookup outcome. Reconciliation outcomes: matched → `mark_send_failed` + INFO; single-batch fallback (1 claim + 1 error, no email match) → `mark_send_failed` + WARNING; unmatched multi-batch → `swallowed += 1` + WARNING + `errors.append("push_leads_unmatched:<code>")`; `mark_send_failed.matched=False` → `errors.append("mark_send_failed_noop:<code>")`. `run_tick` body wrapped in `try/finally` so `dispatch_tick summary` INFO log emits on EVERY return path (7+ early exits like `db_client_unavailable`, `runtime_cap_after_*`, `dispatcher_unavailable`, `batch_fetch_failed`). One auditable JSON line per cron run. `mark_send_failed` predicate is `'dispatching'` (PR #369), not `'pending'`.
...
- **Schema drift CHECK-allowlist pairing rule** (PR #377): `schema_drift_check.py` compares live DB `pg_constraint` names against a hand-maintained Python set `EXPECTED_CHECK_CONSTRAINTS` (NOT parsed from `supabase_schema.sql`). Adding a CHECK to the SQL file WITHOUT appending its name to the Python dict in the same PR = silent CI break on every subsequent PR with "CHECK constraints present in DB but not declared in schema". PRs #353/#356/#366 fell into this trap; #377 retroactively appended the 6 missing names. Also: regex/IN-list CHECK literals MUST use `E''`-prefix to survive shell/Management-API re-quoting (see [[bug_constraint_apostrophe_double_escape_2026-05-27]] memory).
