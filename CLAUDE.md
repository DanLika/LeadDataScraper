...
- **AI-quota 503 pair** (PR #420): `BudgetExceededError` (our SQLite daily cap) → `{"error":"AI daily budget exhausted"}`; `AIQuotaExceededError` (`src/errors.py`; upstream `google.genai.errors.ClientError(code=429)`) → `{"error":"ai_quota_exceeded","retry_after":"tomorrow"}`. Both MUST be re-raised ahead of any `except Exception` in `src/core/agentic_router.py` inner handlers — otherwise the generic catch swallows the typed signal and the backend re-raise at `/ask`+`/insights` is a no-op. SDK class is `google.genai.errors.ClientError`, NOT old-SDK `google.api_core.exceptions.ResourceExhausted`. Runbook: [`docs/runbooks/gemini-quota-exhausted.md`](docs/runbooks/gemini-quota-exhausted.md). Pinned: `tests/unit/test_gemini_budget_endpoint.py::TestAIQuotaExceededExceptionHandler` + `test_guarded_generate_content.py::TestQuotaExceededHandling`.
...
- **Cursor pagination `/leads`**: `?limit=1..200` + `?cursor=<base64url(json({c,k}))>`. Decoder fail-closed → page-1. ≤512 bytes raw, k ≤128 + `_CURSOR_KEY_PATTERN = re.compile(r"\A[A-Za-z0-9_-]{1,128}\Z")` charset gate (k interpolates raw into `.or_()` at `src/utils/supabase_helper.py:178`; `,/)/(` would escape tie-break). `list_leads_recent`: `created_at.lt.<c>` OR `and(created_at.eq.<c>, unique_key.lt.<k>)`. Index `idx_leads_created_at_desc`. Pinned: `tests/security/test_cursor_escape.py` (17 reject + 1 accept; PR #418, closes 2026-05-29 deep-RLS-audit INFO-1).
